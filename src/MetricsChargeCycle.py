...
def _default_get_charge_cycle(dic):
    return dic.get("pico", None)

class MetricsChargeCycle(MetricSample):
    def buildTables(self):
        ...
        self.sampleTable["table"] = Table(
            ...
            sqlalchemy.Column('VBUS_sec',   sqlalchemy.Float()),
            sqlalchemy.Column('VSYS_sec',   sqlalchemy.Float())
            )
    ...
    def processDevice(self, device, sensor, getSample=_default_get_charge_cycle, ts=None):
        if ts == None:
    ...
    def aggregate(self, device, sensor, todoDf, table):    
        ...
        for index, row in todoDf.iterrows():
            ...
            stmt = select(
              func.max(self.sampleTable["table"].c.VBUS_sec),
              func.max(self.sampleTable["table"].c.VSYS_sec)
            ).where(
